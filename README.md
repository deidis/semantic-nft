...
In the context of an artwork metadata (as we want to put correct things into the image itself) we prescribe the tags in one or more `art-.....toml` files.
...
By the way, ExifTool processes tags case-insensitive, so no stress when writing `.toml`.
...
For privacy purposes and cleanness, we recommend to **clear all the metadata** from the image and add what's necessary in the `.toml` flile(s). However, one can also take a different approach, and actually write/clear only specific tags.
